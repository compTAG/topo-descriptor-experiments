...
def generate_random_points_graphs(graph_paths):
    graph_paths.make_dirs()
    ...
    for g_size in k:
        ...
        for pc in pcs:
            ...
            draw_graph(G, G.graph['stratum'], os.path.join(graph_paths.image_dir, output_file))
            nx.write_gpickle(G, os.path.join(graph_paths.graph_dir, str(output_file)+".gpickle"))
            print(output_file+ ": " +str(time.time() - t)+ "(s)")
            t = time.time()


def generate_mpeg7_graphs(eps, data_dir, graph_paths):
    graph_paths.make_dirs()
    ...
    for f in os.listdir(data_dir):
        if f.endswith(".gif") and f.startswith('chicken'):
            # original eps is .005
            print("Starting on graph: "+str(f))
            G, ret = get_img_data_approx(get_mpegSeven_img(data_dir, f),eps,0)
            ...
            print(output_file+ ": " +str(time.time() - t)+ "(s)")
            t = time.time()
    with open(os.path.join(graph_paths.not_used_filename),"w+") as f:
