...
def error_exp(G, outFile):
	with open("output_001_approx/error_exp/"+outFile, "w+") as f:
		...
		f.write("%d,%.16f,%.16f"%(len(G.nodes()), width, height))



def main():
	data_type = 3 #1 for random, 2 for mpeg7, 3 for mnist, 4 for all data types
